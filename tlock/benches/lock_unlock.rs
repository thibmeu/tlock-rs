...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("lock unlock with TLE", |b| b.iter(|| {
        let msg = vec![8; 32];
        let ct = tlock::time_lock(black_box(&pk_bytes), black_box(1000), black_box(msg.clone()));
        let pt = tlock::time_unlock(black_box(&signature), black_box(&ct));
    }));
}
