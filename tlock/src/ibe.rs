...
pub fn public_key(public_key_bytes: &[u8]) -> anyhow::Result<G1Affine> {
    let bytes = public_key_bytes
        .try_into()
        .map_err(|_| anyhow!("invalid public key size"))?;
    Ok(G1Affine::from_compressed(bytes).unwrap())
}
...
pub fn encrypt<I: AsRef<[u8]>, M: AsRef<[u8]>>(master: G1Affine, id: I, msg: M) -> Ciphertext {
    ...
    let gid = {
        let qid = G2Projective::hash::<ExpandMsgXmd<Sha256>>(id.as_ref(), H2C_DST).to_affine();
